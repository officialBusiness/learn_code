...
export default class Core {
	...
	init(dom, statusBar, canvas, images, sounds, coreData) {
		core.dom = dom;
	...
	setStartProgressVal(val) {
		core.dom.startTopProgress.style.width = val + '%';
	}

	setStartLoadTipText(text) {
	...
	resetStatus(hero, hard, floorId, maps) {
	...
	drawHero(direction, x, y, status, offsetX, offsetY) {
		offsetX = offsetX || 0;
		offsetY = offsetY || 0;
	...
	// 开门
	openDoor(id, x, y, needKey, callback) {
		// 是否存在门
	...
	// 楼层切换
	changeFloor(floorId, stair, heroLoc, callback) {
		core.lockControl();
		core.stopHero();
		core.stopAutomaticRoute();
	...
	// 地图切换
	mapChangeAnimate(mode, callback) {
		if (mode == 'show') {
	...
	fillText(map, text, x, y, style, font) {
		if (core.isset(style)) {
	...
	fillRect(map, x, y, width, height, style) {
		if (core.isset(style)) {
			core.setFillStyle(map, style);
		}
	...
	strokeRect(map, x, y, width, height, style, lineWidth) {
		if (core.isset(style)) {
	...
	drawBlock(map, image, cutX, cutY, x, y, size, zoom, clear) {
		zoom = zoom || 1;
	...
	setTrigger(x, y, map, triggerName) {
		var mapBlocks = core.status.thisMap.blocks;
		for (var b = 0; b < mapBlocks.length; b++) {
	...
	addGlobalAnimate(animateMore, x, y, map, loc, size, image) {
		if (animateMore == 2) {
	...
	/*
		removeBlock = function(itemX, itemY) {
			 var mapBlocks = core.status.thisMap.blocks;
			 for(var b = 0;b < mapBlocks.length;b++) {
	...
	*/
	...
	getItem(itemId, itemNum, itemX, itemY, callback) {
	...
	checkStatus(name, need, item, clearData) {
	...
	setStatus(statusName, statusVal) {
