...
export default function Effect(baseName, attributesNames, uniformsNames, samplers, engine, defines) {
    this._engine = engine;
    this.name = baseName;
    ...
    // Is in local store ?
    if (Effect.ShadersStore[baseName + "VertexShader"]) {
        this._prepareEffect(Effect.ShadersStore[baseName + "VertexShader"], Effect.ShadersStore[baseName + "PixelShader"], attributesNames, defines);
    } else {
        var shaderUrl = Engine.ShadersRepository + baseName;
        // Vertex shader
...
// Methods
Effect.prototype._prepareEffect = function (vertexSourceCode, fragmentSourceCode, attributesNames, defines) {
