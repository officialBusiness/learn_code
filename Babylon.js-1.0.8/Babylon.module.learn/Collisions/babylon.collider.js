...
import CollisionPlane from './babylon.collisionPlane.js';
...
Collider.prototype._initialize = function (source, dir, e) {
	this.velocity = dir;
	this.normalizedVelocity = Vector3.Normalize(dir);
...
var checkPointInTriangle = function (point, pa, pb, pc, n) {
	...
	var d = Vector3.Dot(Vector3.Cross(e0, e1), n);
	if (d < 0)
		return false;
...
var intersectBoxAASphere = function (boxMin, boxMax, sphereCenter, sphereRadius) {
	var boxMinSphere = new Vector3(sphereCenter.X - sphereRadius, sphereCenter.Y - sphereRadius, sphereCenter.Z - sphereRadius);
	var boxMaxSphere = new Vector3(sphereCenter.X + sphereRadius, sphereCenter.Y + sphereRadius, sphereCenter.Z + sphereRadius);
...
Collider.prototype._testTriangle = function (subMesh, p1, p2, p3) {
	var t0;
	...
	if (!found) {
		...
		var b = 2.0 * (Vector3.Dot(this.velocity, this.basePoint.subtract(p1)));
		...
		b = 2.0 * (Vector3.Dot(this.velocity, this.basePoint.subtract(p2)));
		...
		b = 2.0 * (Vector3.Dot(this.velocity, this.basePoint.subtract(p3)));
		...
		var edgeDotVelocity = Vector3.Dot(edge, this.velocity);
		var edgeDotBaseToVertex = Vector3.Dot(edge, baseToVertex);

		a = edgeSquaredLength * (-velocitySquaredLength) + edgeDotVelocity * edgeDotVelocity;
		b = edgeSquaredLength * (2.0 * Vector3.Dot(this.velocity, baseToVertex)) - 2.0 * edgeDotVelocity * edgeDotBaseToVertex;
		...
		edgeDotVelocity = Vector3.Dot(edge, this.velocity);
